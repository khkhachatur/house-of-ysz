...
export default function CategoryProducts({ products, title }: { products: CategoryProductRow[]; title: string }) {
  const [showFilters, setShowFilters] = useState(false);
  const [sizes, setSizes] = useState<Set<string>>(new Set());
  const [colors, setColors] = useState<Set<string>>(new Set());
  const [inStockOnly, setInStockOnly] = useState(false);
  const [sortIdx, setSortIdx] = useState(0);
  const sort: SortKey = SORTS[sortIdx].key;
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 border-b border-gray-200 pb-6 gap-6">
        ...
        <div className="flex items-center gap-8 text-[10px] font-bold tracking-[0.2em] uppercase text-gray-400">
          <button onClick={() => setShowFilters((v) => !v)} className={`transition-colors ${showFilters || activeCount > 0 ? "text-black" : "hover:text-black"}`}>
            Filters {showFilters ? "−" : "+"}{activeCount > 0 ? ` (${activeCount})` : ""}
          </button>
          <button onClick={() => setSortIdx((i) => (i + 1) % SORTS.length)} className="hover:text-black transition-colors">
            Sort By: {SORTS[sortIdx].label}
          </button>
        </div>
      </div>
        ...
        {showFilters && (
          ...
          >
            <div className="flex flex-wrap gap-x-12 gap-y-6 bg-gray-50 p-6 mb-8">
              ...
              {allColors.length > 1 && (
                <div className="flex flex-col gap-3">
                  <span className="text-[10px] font-bold tracking-[0.2em] uppercase text-gray-400">Color</span>
              ...
              <div className="flex flex-col gap-3">
                <span className="text-[10px] font-bold tracking-[0.2em] uppercase text-gray-400">Availability</span>
