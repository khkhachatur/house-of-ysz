      ...
      {toast && (
        ...
        >
          <div className="relative w-10 h-12 bg-gray-800 overflow-hidden shrink-0">
            <Image src={toast.product.imageSrc} alt={toast.product.name} fill sizes="40px" className="object-cover" />
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-bold tracking-[0.2em] uppercase">Added to bag</span>
            <span className="text-[10px] tracking-widest uppercase text-gray-400">{toast.product.name}</span>
          </div>
        </motion.div>
      )}
    </AnimatePresence>
